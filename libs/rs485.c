...
USARTPREFIX_init()
{
    ...
    UART_deport.DIRSET = UART_depin;
    UART_deport.OUTCLR = UART_depin;
    ...
    uint16_t bsel = (F_CPU - ((((uint32_t)8) << bscale) * UART_baudrate)) /
                    ((((uint32_t)16) << bscale) * UART_baudrate);
    ...
    UART_USART.CTRLA =
      (UART_rxen ? USART_RXCINTLVL_MED_gc : USART_RXCINTLVL_OFF_gc) |
      (UART_txen ? USART_TXCINTLVL_MED_gc : 0x00);
    UART_USART.CTRLB =
      (UART_rxen ? USART_RXEN_bm : 0x00) | (UART_txen ? USART_TXEN_bm : 0x00);
}
...
ISR(UART_DRE_vect)
{
    ...
    if (USARTPREFIX_txidxA == USARTPREFIX_txidxB) {
        // empty
        UART_USART.CTRLA =
          (UART_USART.CTRLA & ~USART_DREINTLVL_gm) | USART_DREINTLVL_OFF_gc;
    } else {
...
USARTPREFIX_putchar(uint8_t ch)
{
    ...
    UART_USART.CTRLA =
      (UART_USART.CTRLA & ~USART_DREINTLVL_gm) | USART_DREINTLVL_MED_gc;
}
